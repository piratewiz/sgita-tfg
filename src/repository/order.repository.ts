...
export class OrderRepository {
  ...
  async findById(id: string): Promise<IOrder | null> {
    return Order.findById(id)
      .select(
        'numberOrder providerId truckId employeeId dateArriveOrder dateRealReception status createdAt'
      )
  ...
  async findOrderLines(orderId: string): Promise<IOrderProduct[]> {
    return OrderProduct.find({ orderId })
      .select(
        'orderId productId expectedQuantity receivedQuantity scannedCode expiredDate'
      )
      .populate('productId', 'name productCode unitType')
      .lean() as unknown as IOrderProduct[]
  }

  async registerReceptionLines(
    orderId: string,
    line: ReceptionLineDto
  ...
  async findByDate(date: string): Promise<IOrder[]> {
    ...
    return Order.find({ createdAt: { $gte: start, $lt: end } })
      .populate('providerId', 'name')
      .populate('truckId', 'licencePlate')
      .lean() as unknown as IOrder[]
  }
}