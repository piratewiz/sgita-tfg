...
export class IncidenceRepository {
  ...
  async findIncidenceByStatus(status: string): Promise<IIncidence[]> {
    return Incidence.find({ status })
      .populate('orderId', 'numberOrder')
      .populate('providerId', 'name').lean() as unknown as IIncidence[];
  }

  async createIncidence(
    data: CreateIncidenceDto,
    employeeId: string
  ): Promise<IIncidence> {
    return Incidence.create({ ...data, employeeId })
  }

  async updateIncidenceStatus(
    id: string,
    data: UpdateIncidenceDto
  ): Promise<IIncidence | null> {
