...
export class ProductRepository {
  async findAll(): Promise<IProduct[]> {
    return Product.find()
      .select(
        'name category productCode unitType quantity minStock status expirationDate providerId updatedAt'
      )
  ...
  async findByIds(ids: string[]): Promise<IProduct[]> {
    return Product.find({
      ...
      .select(
        'name productCode unitType quantity minStock status expirationDate providerId updatedAt'
      )
      .lean() as unknown as IProduct[]
  }
  ...
  async findByCodes(codes: string[]): Promise<IProduct[]> {
    return Product.find({ productCode: { $in: codes } })
      .select(
        'name productCode unitType quantity minStock status expirationDate providerId updatedAt'
      )
      .lean() as unknown as IProduct[]
  ...
  async findMinStock(): Promise<IProduct[]> {
    return Product.find({ $expr: { $lte: ['$quantity', '$minStock'] } })
      .populate('providerId', 'name')
      .lean() as unknown as IProduct[]
  }
  ...
  async bulkUpdateExpirationAndStatus(
    items: { productId: string; expirationDate: Date }[]
  ): Promise<void> {
    ...
    await Product.bulkWrite(
      items.map((item) => {
        ...
        return {
          updateOne: {
            filter: { _id: new Types.ObjectId(item.productId) },
            update: { $set: { expirationDate: expDate, status, updatedAt: now } }
          }
        }
      })
    )
